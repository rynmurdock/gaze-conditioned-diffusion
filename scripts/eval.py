...
def run_eval(
    ...
    while total_scores < n_samples:
        ...
        for sample in tqdm(val_dataloader):
            ...
            for ind in guidance_scales:
                ...
                with_scanpath.save(f'scratch/{total_scores}_{ind}_with_scanpath_pred.png')
    ...
    plot_scores(lpips_scores, score_types=['lpips (lower is better)'], attribute='cfg', save_path=f'{path_to_save_to}/lpips_plot.png')
    plot_scores(cmmd_scores, score_types=['CMMD (lower is better)'], attribute='cfg', save_path=f'{path_to_save_to}/cmmd_plot.png')
    plot_scores(dino_scores, score_types=['DINO Score (higher is better)'], attribute='cfg', save_path=f'{path_to_save_to}/dinoscore_plot.png')
