...
#
# ---------------------------------------------------------------------------
# NOTE: This is a standalone version of diffusers' `transformer_flux2.py`.
# It has been modified to use absolute imports (`diffusers.xxx`) instead of
# the original package-relative imports (`...xxx`, `..xxx`), so that this
# single file can be placed anywhere on disk (e.g. outside the diffusers
# source tree) and still run correctly, as long as the `diffusers` package
# itself is installed (e.g. via `pip install diffusers`).
# ---------------------------------------------------------------------------
...
def _get_projections(attn: "Flux2Attention", hidden_states, encoder_hidden_states=None):
    ...
    return query, key, value, None, None, None
...
class Flux2Transformer2DModel(
    ...
    def forward(
        ...
        """
        num_txt_tokens = 0# encoder_hidden_states.shape[1]
        ...
        concat_rotary_emb = (
            torch.cat([image_rotary_emb[0]], dim=0),
            torch.cat([image_rotary_emb[1]], dim=0),
        )
